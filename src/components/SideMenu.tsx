...
export function SideMenu({ visible, onClose, onNavigate }: { visible: boolean; onClose: () => void; onNavigate: (section: string) => void }) {
  ...
  return <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose} statusBarTranslucent>
    <View style={styles.overlay}><Pressable style={styles.dismiss} onPress={onClose}/><Animated.View style={[styles.drawer, { width, transform: [{ translateX }] }]}>
      ...
      <View style={styles.avatar}><Text style={styles.avatarEmoji}>🏝️</Text></View><Text style={styles.profile}>Viajante</Text><Text style={styles.profileLink}>Seu caminho, seu clima</Text>
...
const styles = StyleSheet.create({ overlay: { flex: 1, flexDirection: 'row', backgroundColor: 'rgba(2,35,37,.38)' }, dismiss: { flex: 1 }, drawer: { backgroundColor: colors.primary, paddingTop: 58, paddingHorizontal: spacing.lg, paddingBottom: 30, shadowColor: '#000', shadowOpacity: .25, shadowRadius: 18, elevation: 20 }, close: { position: 'absolute', right: 18, top: 18 }, avatar: { width: 84, height: 84, borderRadius: 42, alignSelf: 'center', alignItems: 'center', justifyContent: 'center', backgroundColor: '#BDE8E7', borderWidth: 3, borderColor: '#fff' }, avatarEmoji: { fontSize: 46 }, profile: { color: '#fff', textAlign: 'center', fontWeight: '900', fontSize: 22, marginTop: spacing.sm }, profileLink: { color: '#CBE3DF', textAlign: 'center', marginTop: 3 }, menu: { marginTop: spacing.xl, gap: 5, flex: 1 }, item: { minHeight: 50, flexDirection: 'row', alignItems: 'center', borderRadius: radius.md, paddingHorizontal: spacing.md, gap: spacing.md }, active: { backgroundColor: 'rgba(255,255,255,.18)' }, label: { color: '#fff', fontSize: 15, fontWeight: '700' }, logout: { minHeight: 54, flexDirection: 'row', alignItems: 'center', gap: spacing.md, borderRadius: radius.md, paddingHorizontal: spacing.md, backgroundColor: 'rgba(0,25,26,.28)' } });
